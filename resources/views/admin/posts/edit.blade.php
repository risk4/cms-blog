                ...
                <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-6.5">
                        <div class="mb-4.5">
                            ...
                            <input type="text" name="title" value="{{ old('title', $post->title) }}" placeholder="Enter post title" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" required />
                            @error('title')
                        ...
                        <div class="mb-4.5">
                            ...
                            <input type="text" name="slug" value="{{ old('slug', $post->slug) }}" placeholder="Auto-generated from title" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" />
                            @error('slug')
                            <p class="text-meta-1 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4.5">
                            ...
                            <div class="mb-3">
                                <img src="{{ asset('storage/' . $post->featured_image) }}" alt="Current featured image" class="h-32 w-auto rounded border border-stroke" />
                        ...
                        <div class="mb-4.5">
                            ...
                            <input type="datetime-local" name="published_at" value="{{ old('published_at', $post->published_at ? $post->published_at->format('Y-m-d\TH:i') : '') }}" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" />
                            @error('published_at')
                            <p class="text-meta-1 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            <label class="flex cursor-pointer select-none items-center">
                                <div class="relative">
                                    <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $post->is_featured) ? 'checked' : '' }} class="sr-only" />
                                    <div class="box block h-8 w-14 rounded-full {{ old('is_featured', $post->is_featured) ? 'bg-primary' : 'bg-meta-9 dark:bg-[#5A616B]' }}"></div>
                                    <div class="dot absolute left-1 top-1 flex h-6 w-6 items-center justify-center rounded-full bg-white transition {{ old('is_featured', $post->is_featured) ? 'translate-x-full' : '' }}"></div>
                                </div>
