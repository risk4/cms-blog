                ...
                <table class="w-full text-left text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @forelse($pages as $page)
                            <tr x-show="search === '' || '{{ strtolower($page->title) }}'.includes(search.toLowerCase())" 
                                class="border-b border-gray-200 hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-700/50">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        @if($page->featured_image)
                                            <img src="{{ Storage::url($page->featured_image) }}" alt="" class="h-10 w-10 rounded-lg object-cover">
                                        @else
                                        ...
                                        <div>
                                            <div class="font-medium text-gray-900 dark:text-white">
                                                <a href="{{ route('admin.pages.edit', $page) }}" class="hover:text-brand-500">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @else
                                        <span class="inline-flex items-center rounded-full bg-gray-50 px-2.5 py-0.5 text-xs font-medium text-gray-700 dark:bg-gray-700 dark:text-gray-400">
                                            {{ ucfirst($page->status) }}
                                        </span>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-medium text-blue-700 dark:bg-blue-900/20 dark:text-blue-400">
                                        {{ $page->template ?? 'Default' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-gray-500 dark:text-gray-400">
                                    <div class="text-xs">
                                        {{ $page->updated_at?->format('M d, Y') ?? $page->created_at->format('M d, Y') }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <button @click="deleteModal = true; deleteUrl = '{{ route('admin.pages.destroy', $page) }}'; deleteName = '{{ $page->title }}'"
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <a href="{{ route('admin.pages.create') }}" class="mt-4 inline-flex items-center gap-2 rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                                            </svg>
                                            Create Page
